...
export function createElevatorBlockout(scene: Scene, parts: MeasuredObject[]) {
  ...
  return parts.map((part) => {
    const mesh = MeshBuilder.CreateBox(part.id, { width: part.width, depth: part.depth, height: part.height }, scene);
    mesh.position.set(part.position[0], part.position[1], part.position[2]);
    mesh.rotation.set(...part.rotation);
    mesh.material = part.type === "elevator-tower" ? accentMaterial : elevatorMaterial;
...
export async function replaceElevatorWithGlb(scene: Scene) {
  return SceneLoader.ImportMeshAsync("", "/models/", "elevador_lacerda.glb", scene);
}
...
export function createConnectionPoints(scene: Scene, points: MeasuredObject[]) {
  ...
  return points.map((point) => {
    const mesh = point.id === "elevatorPath"
      ? MeshBuilder.CreateCylinder(point.id, { height: point.height, diameter: 1.5 }, scene)
