...
import { PLAYER_EYE_HEIGHT, walkableSurfaceHeight } from "./player";
import type { Point2, SceneLevels, TerrainConfig } from "./types";

export const THIRD_PERSON_PLAYER_HEIGHT = 1.8;
const PLAYER_HEIGHT = THIRD_PERSON_PLAYER_HEIGHT;
...
export interface ThirdPersonPlayerOptions {
  ...
  levels: SceneLevels;
  onPositionChange?: (position: Point2) => void;
  onInteract?: () => void;
}
...
export function thirdPersonMovementDirection(
  ...
  let forwardX = cameraForward[0];
  let forwardZ = cameraForward[1];
...
function keyInput(keys: ReadonlySet<string>): ThirdPersonInputState {
  return {
...
export function createThirdPersonPlayer({
  ...
  player.material = material(scene, "player");
  player.isPickable = false;
  player.receiveShadows = true;
  player.checkCollisions = true;
  player.ellipsoid = new Vector3(PLAYER_RADIUS, PLAYER_HALF_HEIGHT, PLAYER_RADIUS);
  player.ellipsoidOffset = Vector3.Zero();
  player.metadata = {
    category: "third-person-player",
    realHeightMeters: PLAYER_HEIGHT,
  };
  ...
  const surfaceAt = (x: number, z: number) => walkableSurfaceHeight(scene, terrain, levels, x, z);

  const syncToTerrain = () => {
    const surface = surfaceAt(player.position.x, player.position.z);
    player.position.y = surface + PLAYER_HALF_HEIGHT + SURFACE_CLEARANCE;
  };

  const setHorizontalPosition = (x: number, z: number) => {
    player.position.x = x;
    player.position.z = z;
    syncToTerrain();
  ...
  setHorizontalPosition(spawn[0], spawn[1]);
  ...
  const observer = scene.onBeforeRenderObservable.add(() => {
    ...
    if (Math.abs(moveX) > 0.000001 || Math.abs(moveZ) > 0.000001) {
      ...
      player.moveWithCollisions(new Vector3(moveX * speed * seconds, 0, moveZ * speed * seconds));
      syncToTerrain();
      player.rotation.y = Math.atan2(moveX, moveZ);
      onPositionChange?.([player.position.x, player.position.z]);
    }
  });
  ...
  return {
    ...
    syncStreetCamera: (street: UniversalCamera) => {
      const surface = surfaceAt(player.position.x, player.position.z);
      street.position.set(
        player.position.x,
        surface + PLAYER_EYE_HEIGHT + SURFACE_CLEARANCE,
        player.position.z,
      );

      const forward = new Vector3(Math.sin(player.rotation.y), 0, Math.cos(player.rotation.y));
      street.setTarget(street.position.add(forward));
    },
    dispose: () => {
